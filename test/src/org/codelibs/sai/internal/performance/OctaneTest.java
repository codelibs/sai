...
package org.codelibs.sai.internal.performance;
...
public class OctaneTest {
    ...
    public void genericTest(final String benchmark) {
        try {
            ...
            final String[] args = { "--", benchmarkScript, "--verbose" };
            final Double score = genericSaiTest(benchmark, mainScript, args);
            ...
            if (checkV8Presence()) {
                v8Score =
                        genericV8Test(benchmark,
                                "test/script/basic/run-octane.js -- test/script/external/octane/benchmarks/" + benchmark.toLowerCase()
                                        + ".js --verbose");
            }
    ...
    public Double genericSaiTest(final String benchmark, final String testPath, final String[] args) throws Throwable {
        try {
            ...
            final URL testURL = absoluteFile.toURL();
    ...
    void generateOutput(final String benchmark, final Double sai, final Double rhino, final Double v8) throws Exception {
        ...
        if (rhino != null && rhino != 0) {
            saiToRhino = sai.doubleValue() / rhino.doubleValue();
        }
        if (v8 != null && rhino != 0) {
            saiToV8 = sai.doubleValue() / v8.doubleValue();
        }
        final String normalizedBenchmark = benchmark.replace("-", "");
        System.out.println("benchmark-" + normalizedBenchmark + "-sai=" + sai);
        AuroraWrapper.addResults(AuroraWrapper.createOrOpenDocument(), "benchmark-" + normalizedBenchmark + "-sai", sai.toString());
        ...
        if (saiToRhino != null) {
            System.out.println("benchmark-" + normalizedBenchmark + "-sai-to-rhino=" + ((float) ((int) (saiToRhino * 100))) / 100);
            AuroraWrapper.addResults(AuroraWrapper.createOrOpenDocument(), "benchmark-" + normalizedBenchmark + "-sai-to-rhino", ""
                    + ((float) ((int) (saiToRhino * 100))) / 100);
        }
        if (saiToV8 != null) {
            System.out.println("benchmark-" + normalizedBenchmark + "-sai-to-v8=" + ((float) ((int) (saiToV8 * 100))) / 100);
            AuroraWrapper.addResults(AuroraWrapper.createOrOpenDocument(), "benchmark-" + normalizedBenchmark + "-sai-to-v8", ""
                    + ((float) ((int) (saiToV8 * 100))) / 100);
        }
    }
