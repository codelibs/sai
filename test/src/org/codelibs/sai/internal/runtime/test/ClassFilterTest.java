...
import org.codelibs.sai.api.scripting.URLReader;
import org.codelibs.sai.internal.test.framework.TestFinder;
import org.testng.annotations.Test;

@SuppressWarnings("javadoc")
public class ClassFilterTest {
    private static final String SAI_CODE_CACHE = "sai.persistent.code.cache";
    private static final String CLASSFILTER_CODE_CACHE = "build/classfilter_sai_code_cache";
    ...
    public void runExternalJsTest() {
        final String[] paths =
                new String[] { "test/script/basic/compile-octane.js", "test/script/basic/jquery.js", "test/script/basic/prototype.js",
                        "test/script/basic/runsunspider.js", "test/script/basic/underscore.js", "test/script/basic/yui.js",
                        "test/script/basic/run-octane.js" };
        final SaiScriptEngineFactory factory = new SaiScriptEngineFactory();
        for (final String path : paths) {
            final ScriptEngine engine =
                    factory.getScriptEngine(new String[] { "-scripting" }, getClass().getClassLoader(), getClassFilter());
            try {
    ...
    public void persistentCacheTest() {
        final String oldCodeCache = System.getProperty(SAI_CODE_CACHE);
        System.setProperty(SAI_CODE_CACHE, CLASSFILTER_CODE_CACHE);
        try {
            persistentCacheTestImpl();
        } finally {
            if (oldCodeCache != null) {
                System.setProperty(SAI_CODE_CACHE, oldCodeCache);
            }
        }
    }
    ...
    private void persistentCacheTestImpl() {
        ...
        final ScriptEngine engine =
                factory.getScriptEngine(
                        ...
                        getClass().getClassLoader(), getClassFilter());
        ...
        final ScriptEngine engineSafe =
                factory.getScriptEngine(
                        TestFinder.addExplicitOptimisticTypes(new String[] { "--persistent-code-cache", "--optimistic-types=true" }),
                        getClass().getClassLoader(), new ClassFilter() {
                            @Override
    ...
    private static String generateCodeForPersistentStore() {
        ...
        for (int i = 0; i < 100; i++) {
            stringBuilder.append("function i").append(i).append("(y, z) { var x").append(i).append(" = ").append(i).append(";}");
        }
