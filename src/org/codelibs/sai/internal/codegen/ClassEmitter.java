...
import static org.codelibs.sai.internal.codegen.CompilerConstants.GET_ARRAY_SUFFIX;
...
import static org.codelibs.sai.internal.codegen.CompilerConstants.INIT;
import static org.codelibs.sai.internal.codegen.CompilerConstants.SET_MAP;
...
import org.codelibs.sai.internal.ir.debug.SaiClassReader;
 ...
 * environment "sai.codegen.debug" to true, or --log=codegen:{@literal <level>}
 *
 * @see Compiler
 */
public class ClassEmitter {
    ...
    ClassEmitter(final Context context, final String sourceName, final String unitClassName, final boolean strictMode) {
        ...
        cw.visit(V1_7, ACC_PUBLIC | ACC_SUPER, unitClassName, null, pathName(org.codelibs.sai.internal.scripts.JS.class.getName()), null);
    ...
    private void defineCommonUtilities() {
        ...
        if (constantMethodNeeded.contains(String.class)) {
            ...
            final MethodEmitter getStringMethod =
                    method(EnumSet.of(Flag.PRIVATE, Flag.STATIC), GET_STRING.symbolName(), String.class, int.class);
            getStringMethod.begin();
            getStringMethod.getStatic(unitClassName, CONSTANTS.symbolName(), CONSTANTS.descriptor()).load(Type.INT, 0).arrayload()
                    .checkcast(String.class)._return();
            getStringMethod.end();
        }
        ...
        if (constantMethodNeeded.contains(PropertyMap.class)) {
            ...
            final MethodEmitter getMapMethod =
                    method(EnumSet.of(Flag.PUBLIC, Flag.STATIC), GET_MAP.symbolName(), PropertyMap.class, int.class);
            getMapMethod.begin();
            getMapMethod.loadConstants().load(Type.INT, 0).arrayload().checkcast(PropertyMap.class)._return();
            getMapMethod.end();

            // $setMap - overwrite an existing map.
            final MethodEmitter setMapMethod =
                    method(EnumSet.of(Flag.PUBLIC, Flag.STATIC), SET_MAP.symbolName(), void.class, int.class, PropertyMap.class);
            setMapMethod.begin();
    ...
    private void defineGetArrayMethod(final Class<?> clazz) {
        ...
        getArrayMethod.getStatic(unitClassName, CONSTANTS.symbolName(), CONSTANTS.descriptor()).load(Type.INT, 0).arrayload()
                .checkcast(clazz).invoke(virtualCallNoLookup(clazz, "clone", Object.class)).checkcast(clazz)._return();
        getArrayMethod.end();
    }
    ...
    public void end() {
        ...
        if (unitClassName != null) {
            ...
            initMethod.newInstance(org.codelibs.sai.internal.scripts.JS.class);
    ...
    static String disassemble(final byte[] bytecode) {
        ...
        try (final PrintWriter pw = new PrintWriter(baos)) {
            final SaiClassReader cr = new SaiClassReader(bytecode);
            ...
            final TraceClassVisitor tcv = new TraceClassVisitor(null, new SaiTextifier(ctx.getEnv(), cr), pw);
            cr.accept(tcv, 0);
        }
    ...
    MethodEmitter method(final FunctionNode functionNode) {
        ...
        final MethodVisitor mv =
                cw.visitMethod(ACC_PUBLIC | ACC_STATIC | (functionNode.isVarArg() ? ACC_VARARGS : 0), functionNode.getName(),
                        signature.toString(), null, null);
    ...
    MethodEmitter restOfMethod(final FunctionNode functionNode) {
        ...
        methodNames.add(functionNode.getName());
        final MethodVisitor mv =
                cw.visitMethod(ACC_PUBLIC | ACC_STATIC, functionNode.getName(),
                        Type.getMethodDescriptor(functionNode.getReturnType().getTypeClass(), RewriteException.class), null, null);
