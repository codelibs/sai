...
import static org.codelibs.sai.internal.codegen.CompilerConstants.EXPLODED_ARGUMENT_PREFIX;
...
public final class ApplySpecialization extends SimpleNodeVisitor implements Loggable {
    ...
    private static final boolean USE_APPLY2CALL = Options.getBooleanProperty("sai.apply2call", true);
    ...
    public Node leaveCallNode(final CallNode callNode) {
        ...
        if (isApply(callNode)) {
            ...
            if (log.isEnabled()) {
                log.fine("Transformed ", callNode, " from apply to call => ", newCallNode, " in ",
    ...
    private void pushExplodedArgs(final FunctionNode functionNode) {
        ...
        assert actualCallSiteType.parameterType(actualCallSiteType.parameterCount() - 1) != Object[].class : "error vararg callsite passed to apply2call "
                + functionNode.getName() + " " + actualCallSiteType;
    ...
    public boolean enterFunctionNode(final FunctionNode functionNode) {
        ...
        if (log.isEnabled()) {
            log.info("Trying to specialize apply to call in '", functionNode.getName(), "' params=", functionNode.getParameters(), " id=",
                    functionNode.getId(), " source=", massageURL(functionNode.getSource().getURL()));
        }
    ...
    public Node leaveFunctionNode(final FunctionNode functionNode) {
        ...
        if (changed.contains(newFunctionNode.getId())) {
            newFunctionNode =
                    newFunctionNode.clearFlag(lc, FunctionNode.USES_ARGUMENTS).setFlag(lc, FunctionNode.HAS_APPLY_TO_CALL_SPECIALIZATION)
                            .setParameters(lc, explodedArguments.peek());

            if (log.isEnabled()) {
                log.info("Success: ", massageURL(newFunctionNode.getSource().getURL()), '.', functionName, "' id=",
                        newFunctionNode.getId(), " params=", callSiteTypes.peek());
            }
        }
