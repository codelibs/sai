...
import static org.codelibs.sai.internal.codegen.Compiler.SCRIPTS_PACKAGE;
import static org.codelibs.sai.internal.codegen.CompilerConstants.ALLOCATE;
import static org.codelibs.sai.internal.codegen.CompilerConstants.INIT_ARGUMENTS;
import static org.codelibs.sai.internal.codegen.CompilerConstants.INIT_MAP;
import static org.codelibs.sai.internal.codegen.CompilerConstants.INIT_SCOPE;
import static org.codelibs.sai.internal.codegen.CompilerConstants.JAVA_THIS;
import static org.codelibs.sai.internal.codegen.CompilerConstants.JS_OBJECT_DUAL_FIELD_PREFIX;
import static org.codelibs.sai.internal.codegen.CompilerConstants.JS_OBJECT_SINGLE_FIELD_PREFIX;
import static org.codelibs.sai.internal.codegen.CompilerConstants.className;
import static org.codelibs.sai.internal.codegen.CompilerConstants.constructorNoLookup;
import static org.codelibs.sai.internal.lookup.Lookup.MH;
import static org.codelibs.sai.internal.runtime.JSType.CONVERT_OBJECT;
import static org.codelibs.sai.internal.runtime.JSType.CONVERT_OBJECT_OPTIMISTIC;
import static org.codelibs.sai.internal.runtime.JSType.GET_UNDEFINED;
import static org.codelibs.sai.internal.runtime.JSType.TYPE_DOUBLE_INDEX;
import static org.codelibs.sai.internal.runtime.JSType.TYPE_INT_INDEX;
import static org.codelibs.sai.internal.runtime.JSType.TYPE_OBJECT_INDEX;
import static org.codelibs.sai.internal.runtime.JSType.TYPE_UNDEFINED_INDEX;
import static org.codelibs.sai.internal.runtime.JSType.getAccessorTypeIndex;
...
import org.codelibs.sai.internal.runtime.AllocationStrategy;
import org.codelibs.sai.internal.runtime.Context;
import org.codelibs.sai.internal.runtime.FunctionScope;
...
public final class ObjectClassGenerator implements Loggable {
    ...
    private static final MethodHandle GET_DIFFERENT = findOwnMH("getDifferent", Object.class, Object.class, Class.class,
    ...
    private static void newAllocate(final String className, final ClassEmitter classEmitter) {
        final MethodEmitter allocate =
                classEmitter.method(EnumSet.of(Flag.PUBLIC, Flag.STATIC), ALLOCATE.symbolName(), ScriptObject.class, PropertyMap.class);
        allocate.begin();
    ...
    public static final MethodHandle PACK_DOUBLE = MH.explicitCastArguments(
            MH.findStatic(MethodHandles.publicLookup(), Double.class, "doubleToRawLongBits", MH.type(long.class, double.class)),
            MH.type(long.class, double.class));

    /** double bits to long, used with --dual-fields for primitive double values */
    public static final MethodHandle UNPACK_DOUBLE = MH.findStatic(MethodHandles.publicLookup(), Double.class, "longBitsToDouble",
            MH.type(double.class, long.class));

    //type != forType, so use the correct getter for forType, box it and throw
    @SuppressWarnings("unused")
    private static Object getDifferent(final Object receiver, final Class<?> forType, final MethodHandle primitiveGetter,
            final MethodHandle objectGetter, final int programPoint) {
    ...
    private static MethodHandle createGetterInner(final Class<?> forType, final Class<?> type, final MethodHandle primitiveGetter,
            final MethodHandle objectGetter, final List<MethodHandle> converters, final int programPoint) {
        ...
        if (forType == null) {
            if (isOptimistic) {
                ...
                //throw exception
                return MH.asType(MH.dropArguments(MH.insertArguments(GET_DIFFERENT_UNDEFINED, 0, programPoint), 0, Object.class), getter
                        .type().changeReturnType(type));
            }
        ...
        if (isOptimistic) {
            if (fti < ti) {
                ...
                return MH.asType(MH.filterArguments(objectGetter, 0,
                        MH.insertArguments(GET_DIFFERENT, 1, forType, primitiveGetter, objectGetter, programPoint)), objectGetter.type()
     ...
     */
    public static MethodHandle createGetter(final Class<?> forType, final Class<?> type, final MethodHandle primitiveGetter,
            final MethodHandle objectGetter, final int programPoint) {
        return createGetterInner(forType, type, primitiveGetter, objectGetter, isValid(programPoint) ? CONVERT_OBJECT_OPTIMISTIC
                : CONVERT_OBJECT, programPoint);
    }
     ...
     */
    public static MethodHandle createSetter(final Class<?> forType, final Class<?> type, final MethodHandle primitiveSetter,
     ...
     */
    public static MethodHandle createGuardBoxedPrimitiveSetter(final Class<?> forType, final MethodHandle primitiveSetter,
            final MethodHandle objectSetter) {
        ...
        return MH.guardWithTest(MH.insertArguments(MH.dropArguments(IS_TYPE_GUARD, 1, Object.class), 0, boxedForType),
                MH.asType(primitiveSetter, objectSetter.type()), objectSetter);
    }
