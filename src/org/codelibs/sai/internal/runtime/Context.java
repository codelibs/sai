...
import static org.codelibs.sai.internal.codegen.CompilerConstants.SOURCE;
import static org.codelibs.sai.internal.codegen.CompilerConstants.STRICT_MODE;
import static org.codelibs.sai.internal.runtime.CodeStore.newCodeStore;
...
import org.codelibs.sai.internal.codegen.Compiler.CompilationPhases;
...
import org.codelibs.sai.internal.parser.Parser;
import org.codelibs.sai.internal.runtime.events.RuntimeEvent;
...
public final class Context {
    // sai specific security runtime access permission names
    /**
     * Permission needed to pass arbitrary sai command line options when creating Context.
     */
    public static final String SAI_SET_CONFIG = "sai.setConfig";

    /**
     * Permission needed to create Sai Context instance.
     */
    public static final String SAI_CREATE_CONTEXT = "sai.createContext";

    /**
     * Permission needed to create Sai Global instance.
     */
    public static final String SAI_CREATE_GLOBAL = "sai.createGlobal";

    /**
     * Permission to get current Sai Context from thread local storage.
     */
    public static final String SAI_GET_CONTEXT = "sai.getContext";

    /**
     * Permission to use Java reflection/jsr292 from script code.
     */
    public static final String SAI_JAVA_REFLECTION = "sai.JavaReflection";

    /**
     * Permission to enable sai debug mode.
     */
    public static final String SAI_DEBUG_MODE = "sai.debugMode";

    // sai load psuedo URL prefixes
    private static final String LOAD_CLASSPATH = "classpath:";
    private static final String LOAD_FX = "fx:";
    private static final String LOAD_SAI = "sai:";
     ...
     * and single field representation otherwise. This can be overridden by setting either the "sai.fields.objects"
     * or "sai.fields.dual" system property.
     */
    ...
    public static class ContextCodeInstaller implements CodeInstaller {
        ...
        public void storeScript(final String cacheKey, final Source source, final String mainClassName,
                final Map<String, byte[]> classBytes, final Map<Integer, FunctionInitializer> initializers, final Object[] constants,
    ...
    public static final boolean DEBUG = Options.getBooleanProperty("sai.debug");
    ...
    public static Context getContext() {
        ...
        if (sm != null) {
            sm.checkPermission(new RuntimePermission(SAI_GET_CONTEXT));
        }
    ...
    public Context(final Options options, final ErrorManager errors, final PrintWriter out, final PrintWriter err,
        ...
        final SecurityManager sm = System.getSecurityManager();
        if (sm != null) {
            sm.checkPermission(new RuntimePermission(SAI_CREATE_CONTEXT));
        }
        ...
        if (!env._compile_only && classPath != null && !classPath.isEmpty()) {
            ...
            this.appLoader = SaiLoader.createClassLoader(classPath, appLoader);
        } else {
        ...
        if (env._version) {
            getErr().println("sai " + Version.version());
        }

        if (env._fullversion) {
            getErr().println("sai full version " + Version.fullVersion());
        }

        if (Options.getBooleanProperty("sai.fields.dual")) {
            fieldMode = FieldMode.DUAL;
        } else if (Options.getBooleanProperty("sai.fields.objects")) {
     ...
     */
    public Object eval(final ScriptObject initialScope, final String string, final Object callThis, final Object location,
            final boolean strict, final boolean evalCall) {
        ...
        // Sai extension: any 'eval' is unconditionally strict when -strict is specified.
    ...
    private static Source loadInternal(final String srcStr, final String prefix, final String resourcePath) {
        if (srcStr.startsWith(prefix)) {
            ...
            // NOTE: even sandbox scripts should be able to load scripts in sai: scheme
            // These scripts are always available and are loaded from sai.jar's resources.
    ...
    /**
     * Implementation of {@code load} Sai extension. Load a script file from a source
     * expression
     *
     * @param scope  the scope
    ...
    public Object load(final Object scope, final Object from) throws IOException {
        ...
        if (src instanceof String) {
            ...
            if (srcStr.startsWith(LOAD_CLASSPATH)) {
                ...
                if (srcStr.indexOf(':') != -1) {
                    if ((source = loadInternal(srcStr, LOAD_SAI, "resources/")) == null
    ...
    /**
     * Implementation of {@code loadWithNewGlobal} Sai extension. Load a script file from a source
     ...
     * @see ScriptObject
     *
     * @param fullName  full name of class, e.g. org.codelibs.sai.internal.objects.JO2P1 contains 2 fields and 1 parameter.
     *
     ...
     * {@code org.codelibs.sai.internal.objects.NativeJava} and {@code org.codelibs.sai.internal.runtime.NativeJavaPackage}
     *
     ...
     * @return instrumented method handle, or null if logger not enabled
     */
    public MethodHandle addLoggingToHandle(final Class<? extends Loggable> clazz, final Level level, final MethodHandle mh,
            final int paramStart, final boolean printReturnValue, final Supplier<String> text) {
