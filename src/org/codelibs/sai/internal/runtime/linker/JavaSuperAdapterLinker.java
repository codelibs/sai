...
import static org.codelibs.sai.internal.lookup.Lookup.EMPTY_GETTER;
import static org.codelibs.sai.internal.runtime.linker.JavaAdapterBytecodeGenerator.SUPER_PREFIX;
...
final class JavaSuperAdapterLinker implements TypeBasedGuardingDynamicLinker {
    ...
    public GuardedInvocation getGuardedInvocation(final LinkRequest linkRequest, final LinkerServices linkerServices) throws Exception {
        ...
        final String opName =
                hasFixedName ? (DYN_GET_METHOD_FIXED + descriptor.getNameToken(CallSiteDescriptor.NAME_OPERAND)) : DYN_GET_METHOD;

        final CallSiteDescriptor newDescriptor =
                SaiCallSiteDescriptor.get(descriptor.getLookup(), opName, type.changeParameterType(0, adapterClass), 0);

        // Delegate to BeansLinker
        final GuardedInvocation guardedInv =
                SaiBeansLinker.getGuardedInvocation(BeansLinker.getLinkerForClass(adapterClass),
        ...
        if (guardedInv == null) {
            ...
            // wait().
            return new GuardedInvocation(MethodHandles.dropArguments(EMPTY_GETTER, 1, type.parameterList()
                    .subList(1, type.parameterCount())), guard).asType(descriptor);
        }
        ...
        final MethodHandle typedBinder =
                BIND_DYNAMIC_METHOD.asType(MethodType.methodType(Object.class, invType.returnType(), invType.parameterType(0)));
        // For invocation typed R(T0, T1, ...) create a dynamic method binder of type Object(R, T0, T1, ...)
        final MethodHandle droppingBinder =
                MethodHandles.dropArguments(typedBinder, 2, invType.parameterList().subList(1, invType.parameterCount()));
     ...
     */
    private static MethodHandle asFilterType(final MethodHandle filter, final int pos, final MethodType targetType,
            final MethodType sourceType) {
