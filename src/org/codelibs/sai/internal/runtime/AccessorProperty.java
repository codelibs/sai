...
import static org.codelibs.sai.internal.codegen.ObjectClassGenerator.createGetter;
import static org.codelibs.sai.internal.codegen.ObjectClassGenerator.createSetter;
import static org.codelibs.sai.internal.codegen.ObjectClassGenerator.getFieldCount;
import static org.codelibs.sai.internal.codegen.ObjectClassGenerator.getFieldName;
import static org.codelibs.sai.internal.lookup.Lookup.MH;
import static org.codelibs.sai.internal.lookup.MethodHandleFactory.stripName;
import static org.codelibs.sai.internal.runtime.JSType.getAccessorTypeIndex;
import static org.codelibs.sai.internal.runtime.JSType.getNumberOfAccessorTypes;
...
public class AccessorProperty extends Property {
    ...
    private static final MethodHandle INVALIDATE_SP = findOwnMH_S("invalidateSwitchPoint", Object.class, AccessorProperty.class,
            Object.class);
    ...
    /**
     * Create a new accessor property. Factory method used by saigen generated code.
     *
    ...
    /** Seed getter for the primitive version of this field (in -Dsai.fields.dual=true mode) */
    transient MethodHandle primitiveGetter;

    /** Seed setter for the primitive version of this field (in -Dsai.fields.dual=true mode) */
     ...
     * {@link ScriptRuntime#mergeScope} and the Sai {@code Object.bindProperties} method.
     ...
     * to the constructor, and the rest are created from those. Used e.g. by Saigen classes
     *
     * @param key    the property key
     * @param flags  the property flags
    ...
    private AccessorProperty(final String key, final int flags, final int slot, final MethodHandle getter, final MethodHandle setter) {
        ...
        // we don't need to prep the setters these will never be invalidated as this is a saigen
        ...
        if (getterType == int.class) {
            ...
            primitiveSetter =
                    setter == null ? null : MH.asType(MH.filterArguments(setter, 1, ObjectClassGenerator.UNPACK_DOUBLE),
                            Lookup.SET_PRIMITIVE_TYPE);
        } else {
            primitiveGetter = primitiveSetter = null;
        }

        assert primitiveGetter == null || primitiveGetter.type() == Lookup.GET_PRIMITIVE_TYPE : primitiveGetter + "!="
                + Lookup.GET_PRIMITIVE_TYPE;
    ...
    private void initGetterSetter(final Class<?> structure) {
        ...
        if (isParameter() && hasArguments()) {
            ...
            objectGetter =
                    MH.asType(MH.insertArguments(MH.filterArguments(ScriptObject.GET_ARGUMENT.methodHandle(), 0, arguments), 1, slot),
                            Lookup.GET_OBJECT_TYPE);
            objectSetter =
                    MH.asType(MH.insertArguments(MH.filterArguments(ScriptObject.SET_ARGUMENT.methodHandle(), 0, arguments), 1, slot),
    ...
    public MethodHandle getGetter(final Class<?> type) {
        ...
        if (cachedGetter != null) {
            ...
            getter =
                    debug(createGetter(getLocalType(), type, primitiveGetter, objectGetter, INVALID_PROGRAM_POINT), getLocalType(), type,
                            "get");
    ...
    public MethodHandle getOptimisticGetter(final Class<?> type, final int programPoint) {
        // saigen generated primitive fields like Math.PI have only one known unchangeable primitive type
    ...
    private MethodHandle debug(final MethodHandle mh, final Class<?> forType, final Class<?> type, final String tag) {
        ...
        return context.addLoggingToHandle(ObjectClassGenerator.class, Level.INFO, mh, 0, true, new Supplier<String>() {
            @Override
            public String get() {
                return tag + " '" + getKey() + "' (property=" + Debug.id(this) + ", slot=" + getSlot() + " " + getClass().getSimpleName()
                        + " forType=" + stripName(forType) + ", type=" + stripName(type) + ')';
            }
        });
    }
