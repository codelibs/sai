...
import org.codelibs.sai.internal.runtime.options.KeyValueOption;
import org.codelibs.sai.internal.runtime.options.LoggingOption;
import org.codelibs.sai.internal.runtime.options.LoggingOption.LoggerInfo;
import org.codelibs.sai.internal.runtime.options.Option;
...
public final class ScriptEnvironment {
    ...
    private static final boolean ALLOW_EAGER_COMPILATION_SILENT_OVERRIDE = Options.getBooleanProperty(
            "sai.options.allowEagerCompilationSilentOverride", false);
    ...
    public ScriptEnvironment(final Options options, final PrintWriter out, final PrintWriter err) {
        ...
        if (!lazy_compilation && _optimistic_types) {
            if (!ALLOW_EAGER_COMPILATION_SILENT_OVERRIDE) {
                throw new IllegalStateException(
                        ECMAErrors.getMessage("config.error.eagerCompilationConflictsWithOptimisticTypes",
        ...
        if (options.getBoolean("profile.callsites")) {
            callSiteFlags |= SaiCallSiteDescriptor.CALLSITE_PROFILE;
        }

        if (options.get("trace.callsites") instanceof KeyValueOption) {
            callSiteFlags |= SaiCallSiteDescriptor.CALLSITE_TRACE;
            final KeyValueOption kv = (KeyValueOption) options.get("trace.callsites");
            if (kv.hasValue("miss")) {
                callSiteFlags |= SaiCallSiteDescriptor.CALLSITE_TRACE_MISSES;
            }
            if (kv.hasValue("enterexit") || (callSiteFlags & SaiCallSiteDescriptor.CALLSITE_TRACE_MISSES) == 0) {
                callSiteFlags |= SaiCallSiteDescriptor.CALLSITE_TRACE_ENTEREXIT;
            }
            if (kv.hasValue("objects")) {
                callSiteFlags |= SaiCallSiteDescriptor.CALLSITE_TRACE_VALUES;
            }
        }
