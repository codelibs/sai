...
public final class WithObject extends Scope {
    private static final MethodHandle WITHEXPRESSIONGUARD = findOwnMH("withExpressionGuard", boolean.class, Object.class,
            PropertyMap.class, SwitchPoint[].class);
    ...
    private static final MethodHandle BIND_TO_EXPRESSION_FN = findOwnMH("bindToExpression", Object.class, ScriptFunction.class,
            Object.class);
    ...
    public GuardedInvocation lookup(final CallSiteDescriptor desc, final LinkRequest request) {
        ...
        // With scopes can never be observed outside of Sai code, so all call sites that can address it will of
        // necessity have a Sai descriptor - it is safe to cast.
        final SaiCallSiteDescriptor ndesc = (SaiCallSiteDescriptor) desc;
        ...
        if (self != null) {
            ...
            switch (operator) {
            case "callMethod":
                throw new AssertionError(); // Sai never emits callMethod
            case "getMethod":
    ...
    private static GuardedInvocation fixExpressionCallSite(final SaiCallSiteDescriptor desc, final GuardedInvocation link) {
        ...
        // Make sure getMethod will bind the script functions it receives to WithObject.expression
                MH.foldArguments(
                        ...
                        filterReceiver(linkInvocation.asType(linkType.changeReturnType(
                                linkReturnsFunction ? ScriptFunction.class : Object.class).changeParameterType(0, Object.class)),
                                WITHEXPRESSIONFILTER)), filterGuardReceiver(link, WITHEXPRESSIONFILTER));
    ...
    private GuardedInvocation fixScopeCallSite(final GuardedInvocation link, final String name, final ScriptObject owner) {
        ...
        return link.replaceMethods(filterReceiver(newLink.getInvocation(), WITHSCOPEFILTER),
                SaiGuards.combineGuards(expressionGuard, filterGuardReceiver));
    }
    ...
    private static MethodHandle filterGuardReceiver(final GuardedInvocation link, final MethodHandle receiverFilter) {
        ...
        final MethodHandle filter =
                MH.asType(receiverFilter, receiverFilter.type().changeParameterType(0, receiverType).changeReturnType(receiverType));
