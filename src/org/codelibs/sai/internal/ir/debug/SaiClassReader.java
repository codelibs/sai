 ...
 * as bytecode offsets
 */
public class SaiClassReader extends ClassReader {
     ...
     */
    public SaiClassReader(final byte[] bytecode) {
        super(bytecode);
        parse(bytecode);
    }
    ...
    private static int readInt(final byte[] bytecode, final int index) {
        return ((bytecode[index] & 0xff) << 24) | ((bytecode[index + 1] & 0xff) << 16) | ((bytecode[index + 2] & 0xff) << 8)
    ...
    private void parseCode(final byte[] bytecode, final int index, final int len, final String desc) {
        ...
        for (int i = index; i < index + len;) {
            ...
            labels.add(new SaiLabel(opcode, i - index));
