...
public class PerformanceWrapper extends org.codelibs.sai.tools.Shell {

    int _numberOfIterations;
    int _runsPerIteration;

    protected void runCompileOnlyTest(final String name, final int numberOfIterations, final int runsPerIteration, final String testURL)
            throws Throwable {
        ...
        for (int i = 0; i < numberOfIterations; i++) {
            System.out.println("Iteration " + (i + 1) + " average time: " + ((times[i + 1] - times[i]) / (float) runsPerIteration)
                    / 1000000.0 + " ms.");
        }
    }

    protected void runExecuteOnlyTest(final String name, final int numberOfIterations, final int runsPerIteration, final String testURL)
            throws Throwable {
        runExecuteOnlyTest(name, numberOfIterations, runsPerIteration, testURL, System.out, System.err, new String[0]);
    }

    protected void runExecuteOnlyTest(final String name, final int numberOfIterations, final int runsPerIteration, final String testURL,
            final OutputStream out, final OutputStream err) throws Throwable {
        runExecuteOnlyTest(name, numberOfIterations, runsPerIteration, testURL, out, err, new String[0]);
    }

    protected void runExecuteOnlyTest(final String name, final int numberOfIterations, final int runsPerIteration, final String testURL,
            final OutputStream out, final OutputStream err, final String[] newargs) throws Throwable {
    ...
    protected Object apply(final ScriptFunction target, final Object self) {
        if (_runsPerIteration == 0 && _numberOfIterations == 0) {
            final Global global = org.codelibs.sai.internal.runtime.Context.getGlobal();
            ...
            final java.util.concurrent.ThreadPoolExecutor executor =
                    new java.util.concurrent.ThreadPoolExecutor(1, 1, 1, TimeUnit.MINUTES,
        ...
        for (int i = 0; i < _numberOfIterations; i++) {
            System.out.println("Iteration " + (i + 1) + " average time: " + ((times[i + 1] - times[i]) / (float) _runsPerIteration)
                    / 1000000.0 + " ms.");
        }
