...
import static org.codelibs.sai.internal.test.framework.TestConfig.TEST_JS_FAIL_LIST;
import static org.codelibs.sai.internal.test.framework.TestConfig.TEST_JS_SHARED_CONTEXT;
...
public abstract class AbstractScriptRunnable {
    ...
    public AbstractScriptRunnable(final String framework, final File testFile, final List<String> engineOptions,
    ...
    public String toString() {
        return "Test(compile" + (expectCompileFailure ? "-" : "") + (shouldRun ? ", run" : "") + (expectRunFailure ? "-" : "") + "): "
                + testFile;
    }
    ...
    private static boolean equalsCompilerMsgs(final String es, final String as) {
        ...
        // current environment
        return (split >= 0)
                && as.equals(es.substring(0, split).replaceAll("[/\\\\]", Matcher.quoteReplacement(File.separator)) + es.substring(split));
    }
