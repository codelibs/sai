...
import org.codelibs.sai.internal.test.framework.TestFinder.TestFactory;
...
public class ParallelTestRunner {
    ...
    public static final class ScriptRunnable extends AbstractScriptRunnable implements Callable<ScriptRunnable.Result> {
        ...
        public ScriptRunnable(final String framework, final File testFile, final List<String> engineOptions,
            ...
            try {
                ...
                if (errors != 0 || err.size() > 0) {
                    ...
                    if (!ignoreStdError) {
                        ...
                        try (OutputStream outputFile = new FileOutputStream(getOutputFileName());
                                OutputStream errorFile = new FileOutputStream(getErrorFileName())) {
        ...
        private void copyExpectedFile() {
            ...
            try {
                Files.copy(FileSystems.getDefault().getPath(expectedFileName), FileSystems.getDefault().getPath(getCopyExpectedFileName()),
    ...
    private void suite() throws Exception {
        ...
        final TestFactory<ScriptRunnable> testFactory = new TestFactory<ScriptRunnable>() {
            @Override
            public ScriptRunnable createTest(final String framework, final File testFile, final List<String> engineOptions,
    ...
    public boolean run() throws IOException {
                ...
                if (!result.passed()) {
                    ...
                    if (result.exception != null) {
                        final String exceptionString =
                                result.exception instanceof TestFailedError ? result.exception.getMessage() : result.exception.toString();
        ...
        System.out.printf("Tests run: %d/%d tests, passed: %d (%.2f%%), failed: %d. Time elapsed: %.0fmin %.0fs.\n", doneCount, testCount,
                passCount, 100d * passCount / doneCount, doneCount - passCount, timeElapsed / 60, timeElapsed % 60);
        System.out.flush();
    ...
    private static void setTest262Properties() throws IOException {
        ...
        System.setProperty(TEST_JS_FRAMEWORK,
                "test/script/test262.js test/test262/test/harness/framework.js test/test262/test/harness/sta.js");
