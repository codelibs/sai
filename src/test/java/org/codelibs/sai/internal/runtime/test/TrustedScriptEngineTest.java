...
import org.codelibs.sai.internal.runtime.Version;
import org.testng.annotations.Test;

/**
 * Tests for trusted client usage of sai script engine factory extension API
 */
...
public class TrustedScriptEngineTest {
    ...
    // These are for "private" extension API of SaiScriptEngineFactory that
    ...
    public void factoryClassLoaderTest() {
        ...
        for (final ScriptEngineFactory fac : sm.getEngineFactories()) {
            if (fac instanceof SaiScriptEngineFactory) {
                final SaiScriptEngineFactory nfac = (SaiScriptEngineFactory) fac;
    ...
    public void classFilterTest2() throws ScriptException {
        final SaiScriptEngineFactory fac = new SaiScriptEngineFactory();
        final ScriptEngine e = fac.getScriptEngine(new String[0], Thread.currentThread().getContextClassLoader(), new ClassFilter() {
            @Override
            public boolean exposeToScripts(final String fullName) {
                // don't allow anything that is not "java."
                return fullName.startsWith("java.");
    ...
    @Test
    public void saiSwallowsConstKeyword() throws Exception {
        final SaiScriptEngineFactory f = new SaiScriptEngineFactory();
