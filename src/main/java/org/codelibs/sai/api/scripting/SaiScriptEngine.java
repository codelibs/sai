...
/**
 * JSR-223 compliant script engine for Sai. Instances are not created directly, but rather returned through
 * {@link SaiScriptEngineFactory#getScriptEngine()}. Note that this engine implements the {@link Compilable} and
 * {@link Invocable} interfaces, allowing for efficient precompilation and repeated execution of scripts.
 * @see SaiScriptEngineFactory
 *
 * @since 1.8u40
 */
public final class SaiScriptEngine extends AbstractScriptEngine implements Compilable, Invocable {
    ...
    // underlying sai Context - 1:1 with engine instance
    private final Context saiContext;
    // do we want to share single Sai global instance across ENGINE_SCOPEs?
    private final boolean _global_per_engine;
    // This is the initial default Sai global object.
    // This is used as "shared" global if above option is true.
    private final Global global;

    // Sai script engine error message management
    private static final String MESSAGES_RESOURCE = "org.codelibs.sai.api.scripting.resources.Messages";
    ...
    // helper to get Sai script engine error message
    ...
    SaiScriptEngine(final SaiScriptEngineFactory factory, final String[] args, final ClassLoader appLoader, final ClassFilter classFilter) {
        ...
        // create new Sai Context
        this.saiContext = AccessController.doPrivileged(new PrivilegedAction<Context>() {
            @Override
            public Context run() {
                try {
        ...
        this._global_per_engine = saiContext.getEnv()._global_per_engine;

        // create new global object
        this.global = createSaiGlobal();
    ...
    private <T> T getInterfaceInner(final Object thiz, final Class<T> clazz) {
        ...
        if (thiz == null) {
            ...
            realSelf = realGlobal = getSaiGlobalFrom(context);
            ...
            if (!isOfContext(realGlobal, saiContext)) {
        ...
        try {
            ...
            try {
                ...
                return clazz.cast(JavaAdapterFactory.getConstructor(realSelf.getClass(), clazz, MethodHandles.publicLookup()).invoke(
                        realSelf));
            } finally {
                if (globalChanged) {
    ...
    // Retrieve sai Global object for a given ScriptContext object
    private Global getSaiGlobalFrom(final ScriptContext ctxt) {
        if (_global_per_engine) {
        ...
        // is this Sai's own Bindings implementation?
        ...
        // Arbitrary user Bindings implementation. Look for SAI_GLOBAL in it!
        final Object scope = bindings.get(SAI_GLOBAL);
    ...
    // Retrieve sai Global object from a given ScriptObjectMirror
    private Global globalFromMirror(final ScriptObjectMirror mirror) {
        final ScriptObject sobj = mirror.getScriptObject();
        if (sobj instanceof Global && isOfContext((Global) sobj, saiContext)) {
            return (Global) sobj;
        }
    ...
    // Create a new ScriptObjectMirror wrapping a newly created Sai Global object
    private ScriptObjectMirror createGlobalMirror() {
        final Global newGlobal = createSaiGlobal();
    ...
    private Global createSaiGlobal() {
        ...
        saiContext.initGlobal(newGlobal, this);

        return newGlobal;
    }

    private Object invokeImpl(final Object selfObject, final String name, final Object... args) throws ScriptException,
    ...
    private Object evalImpl(final ScriptFunction script, final ScriptContext ctxt) throws ScriptException {
        return evalImpl(script, ctxt, getSaiGlobalFrom(ctxt));
    }

    private Object evalImpl(final Context.MultiGlobalCompiledScript mgcs, final ScriptContext ctxt, final Global ctxtGlobal)
    ...
    private static void throwAsScriptException(final Exception e, final Global global) throws ScriptException {
        ...
        } else if (e instanceof SaiException) {
            final SaiException ne = (SaiException) e;
    ...
    private CompiledScript asCompiledScript(final Source source) throws ScriptException {
        ...
        final Global oldGlobal = Context.getGlobal();
        final Global newGlobal = getSaiGlobalFrom(context);
        ...
        try {
            ...
            mgcs = saiContext.compileScript(source);
        ...
        return new CompiledScript() {
            ...
            public Object eval(final ScriptContext ctxt) throws ScriptException {
                final Global globalObject = getSaiGlobalFrom(ctxt);
    ...
    private ScriptFunction compileImpl(final Source source, final Global newGlobal) throws ScriptException {
        ...
        try {
            ...
            return saiContext.compileScript(source, newGlobal);
        } catch (final Exception e) {
            throwAsScriptException(e, newGlobal);
            throw new AssertionError("should not reach here");
        } finally {
