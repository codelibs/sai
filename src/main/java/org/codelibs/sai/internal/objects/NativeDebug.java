...
import org.codelibs.sai.internal.runtime.PropertyListeners;
...
/**
 * Sai specific debug utils. This is meant for Sai developers.
...
public final class NativeDebug extends ScriptObject {
    ...
    /**
     * Sai extension: get context, context utility
     *
    ...
    public static Object getContext(final Object self) {
        ...
        if (sm != null) {
            sm.checkPermission(new RuntimePermission(Context.SAI_GET_CONTEXT));
        }
        return Global.getThisContext();
    }

    /**
     * Sai extension: get map from {@link ScriptObject}
     *
     * @param self self reference
    ...
    /**
     * Dump all Sai debug mode counters. Calling this may be better if
    ...
    public static void setEventQueueCapacity(final Object self, final Object newCapacity) {
        ((ScriptObject) self).set(EVENT_QUEUE_CAPACITY, newCapacity, SaiCallSiteDescriptor.CALLSITE_STRICT);
    }
    ...
    private static LinkedList<RuntimeEvent<?>> getEventQueue(final Object self) {
        ...
        if (sobj.has(EVENT_QUEUE)) {
            ...
            ((ScriptObject) self).set(EVENT_QUEUE, q = new LinkedList<>(), SaiCallSiteDescriptor.CALLSITE_STRICT);
        }
        return q;
    }
