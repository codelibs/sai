...
import org.codelibs.sai.internal.parser.DateParser;
...
public final class NativeDate extends ScriptObject {
    ...
    private static int[][] firstDayInMonth = { { 0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334 }, // normal year
    ...
    NativeDate(final double time, final ScriptObject proto) {
        this(time, proto, $saigenmap$);
    }

    NativeDate(final double time, final Global global) {
        this(time, global.getDatePrototype(), $saigenmap$);
    }
    ...
    private static String toStringImpl(final Object self, final int format) {
        ...
        if (nd != null && nd.isValidDate()) {
            ...
            case FORMAT_TIME:
                ...
                sb.append(" (").append(tz.getDisplayName(inDaylightTime, TimeZone.SHORT, Locale.US)).append(')');
                break;
