...
import org.codelibs.sai.internal.ir.Terminal;
import org.codelibs.sai.internal.ir.TernaryNode;
import org.codelibs.sai.internal.ir.annotations.Ignore;
import org.codelibs.sai.internal.ir.annotations.Reference;
...
public final class ASTWriter {
    ...
    private void printAST(final StringBuilder sb, final List<Node> preorder, final Field field, final String name, final Node node,
            final int indent) {
        ...
        if (children.isEmpty()) {
            sb.append("[").append(type).append(' ').append(name).append(" = '").append(node).append("'").append(status).append("] ")
                    .append('\n');
        } else {
            sb.append("[").append(type).append(' ').append(name).append(' ').append(Token.toString(node.getToken())).append(status)
                    .append("]").append('\n');
            ...
            for (final Field child : children) {
                ...
                if (value instanceof Node) {
                    ...
                    sb.append('[').append(child.getName()).append("[0..").append(((Collection<Node>) value).size()).append("]]")
                            .append('\n');
