...
import static org.codelibs.sai.internal.parser.TokenType.BIT_NOT;
...
public final class UnaryNode extends Expression implements Assignment<Expression>, Optimistic {
    ...
    @Ignore
    private static final List<TokenType> CAN_OVERFLOW = Collections.unmodifiableList(Arrays.asList(new TokenType[] { TokenType.ADD,
            TokenType.SUB, //negate
            TokenType.DECPREFIX, TokenType.DECPOSTFIX, TokenType.INCPREFIX, TokenType.INCPOSTFIX, }));
    ...
    public UnaryNode(final long token, final Expression rhs) {
        this(token, Math.min(rhs.getStart(), Token.descPosition(token)), Math.max(Token.descPosition(token) + Token.descLength(token),
                rhs.getFinish()), rhs);
    }
