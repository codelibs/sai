...
/**
 * Utility class shared by {@code SaiLinker} and {@code SaiPrimitiveLinker} for converting JS values to Java
 * types.
 */
final class JavaArgumentConverters {
    ...
    private static void putDoubleConverter(final Class<?> targetType) {
        ...
        CONVERTERS.put(
                primitive,
                MH.explicitCastArguments(JSType.TO_NUMBER.methodHandle(), JSType.TO_NUMBER.methodHandle().type()
                        .changeReturnType(primitive)));
