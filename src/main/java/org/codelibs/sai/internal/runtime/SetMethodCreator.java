 ...
 * {@link ScriptObject#findSetMethod(CallSiteDescriptor, org.codelibs.sai.internal.dynalink.linker.LinkRequest)} and
...
final class SetMethodCreator {
    ...
    private class SetMethod {
        ...
        GuardedInvocation createGuardedInvocation() {
            ...
            final boolean explicitInstanceOfCheck = SaiGuards.explicitInstanceOfCheck(desc, request);
            return new GuardedInvocation(methodHandle, SaiGuards.getGuard(sobj, property, desc, explicitInstanceOfCheck),
    ...
    private void checkStrictCreateNewVariable() {
        ...
        if (SaiCallSiteDescriptor.isScope(desc) && SaiCallSiteDescriptor.isStrict(desc)) {
    ...
    private SetMethod createExistingPropertySetter() {
        ...
        final boolean isStrict = SaiCallSiteDescriptor.isStrict(desc);
        final MethodHandle methodHandle;

        if (SaiCallSiteDescriptor.isDeclaration(desc)) {
    ...
    private SetMethod createNewPropertySetter(final SwitchPoint builtinSwitchPoint) {
        final SetMethod sm =
                map.getFreeFieldSlot() > -1 ? createNewFieldSetter(builtinSwitchPoint) : createNewSpillPropertySetter(builtinSwitchPoint);
    ...
    private SetMethod createNewSetter(final Property property, final SwitchPoint builtinSwitchPoint) {
        ...
        slowSetter = MH.insertArguments(slowSetter, 3, SaiCallSiteDescriptor.getFlags(desc));
    ...
    private SetMethod createNewFieldSetter(final SwitchPoint builtinSwitchPoint) {
        return createNewSetter(new AccessorProperty(getName(), getFlags(sobj), sobj.getClass(), getMap().getFreeFieldSlot(), type),
                builtinSwitchPoint);
    }
