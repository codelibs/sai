...
import org.codelibs.sai.internal.dynalink.support.AbstractCallSiteDescriptor;
import org.codelibs.sai.internal.dynalink.support.CallSiteDescriptorFactory;
import org.codelibs.sai.internal.ir.debug.SaiTextifier;
import org.codelibs.sai.internal.runtime.ScriptRuntime;

/**
 * Sai-specific implementation of Dynalink's {@link CallSiteDescriptor}. The reason we have our own subclass is that
 ...
 */
public final class SaiCallSiteDescriptor extends AbstractCallSiteDescriptor {
    ...
    private static final ClassValue<ConcurrentMap<SaiCallSiteDescriptor, SaiCallSiteDescriptor>> canonicals =
            new ClassValue<ConcurrentMap<SaiCallSiteDescriptor, SaiCallSiteDescriptor>>() {
                @Override
                protected ConcurrentMap<SaiCallSiteDescriptor, SaiCallSiteDescriptor> computeValue(final Class<?> type) {
    ...
    /**
     * Function used by {@link SaiTextifier} to represent call site flags in
     * human readable form
    ...
    /**
     * Retrieves a Sai call site descriptor with the specified values. Since call site descriptors are immutable
     ...
     * @param flags Sai-specific call site flags
     * @return a call site descriptor with the specified values.
     */
    public static SaiCallSiteDescriptor get(final MethodHandles.Lookup lookup, final String name, final MethodType methodType,
            final int flags) {
    ...
    private static SaiCallSiteDescriptor get(final MethodHandles.Lookup lookup, final String operator, final String operand,
            final MethodType methodType, final int flags) {
        final SaiCallSiteDescriptor csd = new SaiCallSiteDescriptor(lookup, operator, operand, methodType, flags);
        ...
        final ConcurrentMap<SaiCallSiteDescriptor, SaiCallSiteDescriptor> classCanonicals = canonicals.get(lookup.lookupClass());
        final SaiCallSiteDescriptor canonical = classCanonicals.putIfAbsent(csd, csd);
        return canonical != null ? canonical : csd;
    }

    private SaiCallSiteDescriptor(final MethodHandles.Lookup lookup, final String operator, final String operand,
            final MethodType methodType, final int flags) {
        this.lookup = lookup;
     ...
     * operation {@code "getProp|getMethod|getElem"}, it will return {@code "getProp"}. Sai - being a ECMAScript
    ...
    public static String getFunctionDescription(final CallSiteDescriptor desc) {
        return desc instanceof SaiCallSiteDescriptor ? ((SaiCallSiteDescriptor) desc).getFunctionDescription() : null;
    }
    ...
    public static String getFunctionErrorMessage(final CallSiteDescriptor desc, final Object obj) {
        return desc instanceof SaiCallSiteDescriptor ? ((SaiCallSiteDescriptor) desc).getFunctionErrorMessage(obj) : ScriptRuntime
                .safeToString(obj);
    }

    /**
     * Returns the Sai-specific flags for this call site descriptor.
     * @param desc the descriptor. It can be any kind of a call site descriptor, not necessarily a
     * {@code SaiCallSiteDescriptor}. This allows for graceful interoperability when linking Sai with code
     * generated outside of Sai.
     * @return the Sai-specific flags for the call site, or 0 if the passed descriptor is not a Sai call site
     * descriptor.
     */
    public static int getFlags(final CallSiteDescriptor desc) {
        return desc instanceof SaiCallSiteDescriptor ? ((SaiCallSiteDescriptor) desc).flags : 0;
    }
     ...
     * generated outside of Sai.
     * @param flag the tested flag
     * @return true if the flag is set, false otherwise (it will be false if the descriptor is not a Sai call site
     * descriptor).
     */
    ...
    /**
     * Returns true if this descriptor is a Sai call site descriptor and has the {@link  #CALLSITE_SCOPE} flag set.
     * @param desc the descriptor. It can be any kind of a call site descriptor, not necessarily a
     * {@code SaiCallSiteDescriptor}. This allows for graceful interoperability when linking Sai with code
     * generated outside of Sai.
     * @return true if the descriptor is a Sai call site descriptor, and the flag is set, false otherwise.
     */
    ...
    /**
     * Returns true if this descriptor is a Sai call site descriptor and has the {@link  #CALLSITE_FAST_SCOPE} flag set.
     * @param desc the descriptor. It can be any kind of a call site descriptor, not necessarily a
     * {@code SaiCallSiteDescriptor}. This allows for graceful interoperability when linking Sai with code
     * generated outside of Sai.
     * @return true if the descriptor is a Sai call site descriptor, and the flag is set, false otherwise.
    ...
    /**
     * Returns true if this descriptor is a Sai call site descriptor and has the {@link  #CALLSITE_STRICT} flag set.
     * @param desc the descriptor. It can be any kind of a call site descriptor, not necessarily a
     * {@code SaiCallSiteDescriptor}. This allows for graceful interoperability when linking Sai with code
     * generated outside of Sai.
     * @return true if the descriptor is a Sai call site descriptor, and the flag is set, false otherwise.
