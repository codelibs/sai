...
public abstract class ScriptFunctionData implements Serializable {
    ...
    public String toStringVerbose() {
        ...
        sb.append("name='").append(name.isEmpty() ? "<anonymous>" : name).append("' ").append(code.size()).append(" invokers=")
                .append(code);
    ...
    final CompiledFunction getBestInvoker(final MethodType callSiteType, final ScriptObject runtimeScope,
            final Collection<CompiledFunction> forbidden) {
     ...
     */
    abstract CompiledFunction getBest(final MethodType callSiteType, final ScriptObject runtimeScope,
     ...
     * @return the best function for the specified call site type.
     */
    final CompiledFunction getBest(final MethodType callSiteType, final ScriptObject runtimeScope,
            final Collection<CompiledFunction> forbidden) {
    ...
    ScriptFunctionData makeBoundFunctionData(final ScriptFunction fn, final Object self, final Object[] args) {
        ...
        final CompiledFunction bindTarget =
     ...
     */
    private MethodHandle bindInvokeHandle(final MethodHandle originalInvoker, final ScriptFunction targetFn, final Object self,
            final Object[] args) {
        ...
        if (isVarArg(originalInvoker)) {
            ...
            final Object[] boundArgs =
                    new Object[Math.min(originalInvoker.type().parameterCount() - argInsertPos, args.length
                            + (isTargetBound ? 0 : needsCallee ? 2 : 1))];
            int next = 0;
            if (!isTargetBound) {
                if (needsCallee) {
    ...
    private static MethodHandle bindConstructHandle(final MethodHandle originalConstructor, final ScriptFunction fn, final Object[] args) {
        ...
        final MethodHandle calleeBoundConstructor =
                fn.isBoundFunction() ? originalConstructor : MH.dropArguments(MH.bindTo(originalConstructor, fn), 0, ScriptFunction.class);
