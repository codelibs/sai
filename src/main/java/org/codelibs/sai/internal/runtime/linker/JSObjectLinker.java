...
final class JSObjectLinker implements TypeBasedGuardingDynamicLinker {
    ...
    public GuardedInvocation getGuardedInvocation(final LinkRequest request, final LinkerServices linkerServices) throws Exception {
        ...
        if (self instanceof JSObject) {
            ...
            final GuardedInvocation beanInv = saiBeansLinker.getGuardedInvocation(request, linkerServices);
            inv =
                    new GuardedInvocation(beanInv.getInvocation(), SaiGuards.combineGuards(beanInv.getGuard(),
                            SaiGuards.getNotJSObjectGuard()));
        } else {
    ...
    private GuardedInvocation lookup(final CallSiteDescriptor desc, final LinkRequest request, final LinkerServices linkerServices)
        ...
        case "getMethod":
            ...
            return findGetIndexMethod(saiBeansLinker.getGuardedInvocation(request, linkerServices));
        case "setProp":
        case "setElem":
    ...
    private static GuardedInvocation findCallMethod(final CallSiteDescriptor desc) {
        MethodHandle mh = SaiCallSiteDescriptor.isScope(desc) ? JSOBJECT_SCOPE_CALL : JSOBJECT_CALL;
        if (SaiCallSiteDescriptor.isApplyToCall(desc)) {
            mh = MH.insertArguments(JSOBJECT_CALL_TO_APPLY, 0, mh);
        }
        final MethodType type = desc.getMethodType();
        mh =
                type.parameterType(type.parameterCount() - 1) == Object[].class ? mh : MH.asCollector(mh, Object[].class,
    ...
    private static final MethodHandle JSOBJECT_SCOPE_CALL = findOwnMH_S("jsObjectScopeCall", Object.class, JSObject.class, Object.class,
            Object[].class);
    private static final MethodHandle JSOBJECT_CALL_TO_APPLY = findOwnMH_S("callToApply", Object.class, MethodHandle.class, JSObject.class,
