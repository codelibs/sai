...
import org.codelibs.sai.internal.codegen.TypeMap;
...
import org.codelibs.sai.internal.runtime.events.RecompilationEvent;
...
final class CompiledFunction {
    ...
    private static final MethodHandle RELINK_COMPOSABLE_INVOKER = findOwnMH("relinkComposableInvoker", void.class, CallSite.class,
            CompiledFunction.class, boolean.class);
    private static final MethodHandle HANDLE_REWRITE_EXCEPTION = findOwnMH("handleRewriteException", MethodHandle.class,
            CompiledFunction.class, OptimismInfo.class, RewriteException.class);
    private static final MethodHandle RESTOF_INVOKER = MethodHandles.exactInvoker(MethodType.methodType(Object.class,
    ...
    static CompiledFunction createBuiltInConstructor(final MethodHandle invoker, final Specialization specialization) {
        return new CompiledFunction(MH.insertArguments(invoker, 0, false),
                createConstructorFromInvoker(MH.insertArguments(invoker, 0, true)), specialization);
    }
    ...
    CompiledFunction(final MethodHandle invoker, final MethodHandle constructor, final int flags, final MethodType callSiteType,
            final Specialization specialization, final DebugLogger log) {
        ...
        if (specialization != null && specialization.isOptimistic()) {
             ...
             */
            this.invoker =
                    MH.insertArguments(invoker, invoker.type().parameterCount() - 1, UnwarrantedOptimismException.FIRST_PROGRAM_POINT);
    ...
    static CompiledFunction createBuiltInConstructor(final MethodHandle invoker) {
        return new CompiledFunction(MH.insertArguments(invoker, 0, false),
                createConstructorFromInvoker(MH.insertArguments(invoker, 0, true)), null);
    }
    ...
    Class<? extends LinkLogic> getLinkLogicClass() {
        if (isSpecialization()) {
            ...
            assert !LinkLogic.isEmpty(linkLogicClass) : "empty link logic classes should have been removed by saigen";
    ...
    public String toString() {
        ...
        sb.append("[invokerType=").append(invoker.type()).append(" ctor=").append(constructor).append(" weight=").append(weight())
                .append(" linkLogic=").append(linkLogicClass != null ? linkLogicClass.getSimpleName() : "none");
        ...
        if (isValid(callerProgramPoint)) {
            ...
            if (callSiteReturnType.isPrimitive() && handleRewriteException != null) {
                ...
                handleRewriteException =
                        OptimisticReturnFilters.filterOptimisticReturnValue(handleRewriteException, callSiteReturnType, callerProgramPoint);
            }
    ...
    private static MethodHandle handleRewriteException(final CompiledFunction function, final OptimismInfo oldOptimismInfo,
            final RewriteException re) {
    ...
    private synchronized MethodHandle handleRewriteException(final OptimismInfo oldOptInfo, final RewriteException re) {
        if (log.isEnabled()) {
            log.info(new RecompilationEvent(Level.INFO, re, re.getReturnValueNonDestructive()), "caught RewriteException ",
                    re.getMessageShort());
            log.indent();
        }
        ...
        if (!shouldRecompile) {
            ...
            return restOfHandle(effectiveOptInfo,
                    compiler.compile(fn, cached ? CompilationPhases.COMPILE_CACHED_RESTOF : CompilationPhases.COMPILE_ALL_RESTOF),
                    currentOptInfo != null);
        }
        ...
        if (log.isEnabled()) {
            ...
            log.info("Recompiled '", fn.getName(), "' (", Debug.id(this), ") ", canBeDeoptimized ? "can still be deoptimized."
                    : " is completely deoptimized.");
        ...
        final MethodHandle restOf =
                restOfHandle(effectiveOptInfo, compiler.compile(fn, CompilationPhases.GENERATE_BYTECODE_AND_INSTALL_RESTOF),
                        canBeDeoptimized);
    ...
    private MethodHandle restOfHandle(final OptimismInfo info, final FunctionNode restOfFunction, final boolean canBeDeoptimized) {
        ...
        final MethodHandle restOf =
                changeReturnType(
                        info.data.lookupCodeMethod(restOfFunction.getCompileUnit().getCode(),
                                MH.type(restOfFunction.getReturnType().getTypeClass(), RewriteException.class)), Object.class);
    ...
    private static class OptimismInfo {
        ...
        boolean requestRecompile(final RewriteException e) {
            ...
            if (previousFailedType != null && !previousFailedType.narrowerThan(retType)) {
                ...
                log.info("RewriteException for an already invalidated program point ", e.getProgramPoint(), " in ", functionId,
                        ". This is okay for a recursive function invocation, but a bug otherwise.");
