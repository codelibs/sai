...
import org.codelibs.sai.internal.dynalink.support.LinkerServicesImpl;
...
/**
 * This is the main dynamic linker for Sai. It is used for linking all {@link ScriptObject} and its subclasses (this
 * includes {@link ScriptFunction} and its subclasses) as well as {@link Undefined}.
 */
final class SaiLinker implements TypeBasedGuardingDynamicLinker, GuardingTypeConverterFactory, ConversionComparator {
    ...
    private static GuardedInvocation getMirrorConverter(final Class<?> sourceType, final Class<?> targetType) {
        ...
        if (targetType == Map.class || targetType == Bindings.class || targetType == JSObject.class
                || targetType == ScriptObjectMirror.class) {
    ...
    private static final MethodHandle IS_NASHORN_OR_UNDEFINED_TYPE = findOwnMH("isSaiTypeOrUndefined", Boolean.TYPE, Object.class);
    ...
    static {
        final MethodHandle listAdapterCreate =
                new Lookup(MethodHandles.lookup()).findStatic(ListAdapter.class, "create",
    ...
    private static boolean isSaiTypeOrUndefined(final Object obj) {
    ...
    private static MethodHandle findOwnMH(final String name, final Class<?> rtype, final Class<?>... types) {
        return MH.findStatic(MethodHandles.lookup(), SaiLinker.class, name, MH.type(rtype, types));
    }
}
