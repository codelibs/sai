...
import org.codelibs.sai.internal.dynalink.linker.ConversionComparator;
...
/**
 * Internal linker for String, Boolean, and Number objects, only ever used by Sai engine and not exposed to other
 ...
 */
final class SaiPrimitiveLinker implements TypeBasedGuardingDynamicLinker, GuardingTypeConverterFactory, ConversionComparator {
    private static final GuardedTypeConversion VOID_TO_OBJECT = new GuardedTypeConversion(new GuardedInvocation(MethodHandles.constant(
            Object.class, ScriptRuntime.UNDEFINED)), true);
    ...
    private static boolean canLinkTypeStatic(final Class<?> type) {
        return type == String.class || type == Boolean.class || type == ConsString.class || type == Integer.class || type == Double.class
    ...
    public GuardedInvocation getGuardedInvocation(final LinkRequest origRequest, final LinkerServices linkerServices) throws Exception {
        final LinkRequest request = origRequest.withoutRuntimeContext(); // Sai has no runtime context

        final Object self = request.getReceiver();
        final SaiCallSiteDescriptor desc = (SaiCallSiteDescriptor) request.getCallSiteDescriptor();
    ...
    public GuardedTypeConversion convertToType(final Class<?> sourceType, final Class<?> targetType) {
        ...
        return new GuardedTypeConversion(new GuardedInvocation(mh, canLinkTypeStatic(sourceType) ? null : GUARD_PRIMITIVE).asType(mh.type()
                .changeParameterType(0, sourceType)), true);
    }
     ...
     * @return one of {@link org.codelibs.sai.internal.dynalink.linker.ConversionComparator.Comparison} values signifying which
    ...
    private static MethodHandle findOwnMH(final String name, final Class<?> rtype, final Class<?>... types) {
        return MH.findStatic(MethodHandles.lookup(), SaiPrimitiveLinker.class, name, MH.type(rtype, types));
    }
}
