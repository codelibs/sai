...
/**
 * Sai bottom linker; used as a last-resort catch-all linker for all linking requests that fall through all other
 ...
 */
final class SaiBottomLinker implements GuardingDynamicLinker, GuardingTypeConverterFactory {
    ...
    public GuardedInvocation getGuardedInvocation(final LinkRequest linkRequest, final LinkerServices linkerServices) throws Exception {
        ...
        // None of the objects that can be linked by SaiLinker should ever reach here. Basically, anything below
        // this point is a generic Java bean. Therefore, reaching here with a ScriptObject is a Sai bug.
    ...
    private static final MethodHandle EMPTY_PROP_SETTER = MH.asType(EMPTY_ELEM_GETTER, EMPTY_ELEM_GETTER.type()
    ...
    private static GuardedInvocation linkBean(final LinkRequest linkRequest, final LinkerServices linkerServices) throws Exception {
        final SaiCallSiteDescriptor desc = (SaiCallSiteDescriptor) linkRequest.getCallSiteDescriptor();
        final Object self = linkRequest.getReceiver();
        ...
        case "setProp":
        case "setElem": {
            final boolean strict = SaiCallSiteDescriptor.isStrict(desc);
            if (strict) {
    ...
    private static GuardedInvocation getInvocation(final MethodHandle handle, final Object self, final LinkerServices linkerServices,
    ...
    // linked by SaiLinker should never reach here.
