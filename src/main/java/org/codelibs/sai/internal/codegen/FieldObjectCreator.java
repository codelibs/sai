...
import static org.codelibs.sai.internal.codegen.ObjectClassGenerator.getPaddedFieldCount;
 ...
 * @see org.codelibs.sai.internal.ir.Node
 */
public abstract class FieldObjectCreator<T> extends ObjectCreator<T> {
    ...
    public void createObject(final MethodEmitter method) {
        ...
        // NOTE: we must load the actual structure class here, because the API operates with Sai Type objects,
    ...
    protected PropertyMap makeMap() {
        ...
        propertyMap =
                newMapCreator(fieldObjectClass).makeFieldMap(hasArguments(), codegen.useDualFields(), fieldCount, paddedFieldCount,
                        evalCode);
        return propertyMap;
    }
    ...
    private void putField(final MethodEmitter method, final String key, final int fieldIndex, final MapTuple<T> tuple) {
        ...
        assert fieldName.equals(getFieldName(fieldIndex, PRIMITIVE_FIELD_TYPE)) || fieldType.isObject() : key
                + " object keys must store to L*-fields";
        assert fieldName.equals(getFieldName(fieldIndex, Type.OBJECT)) || fieldType.isPrimitive() : key
                + " primitive keys must store to J*-fields";
    ...
    private void findClass() {
        fieldObjectClassName =
                isScope() ? ObjectClassGenerator.getClassName(fieldCount, paramCount, codegen.useDualFields()) : ObjectClassGenerator
                        .getClassName(paddedFieldCount, codegen.useDualFields());

        try {
            this.fieldObjectClass = Context.forStructureClass(Compiler.binaryName(fieldObjectClassName));
        } catch (final ClassNotFoundException e) {
            throw new AssertionError("Sai has encountered an internal error.  Structure can not be created.");
        }
    }
     ...
     * e.g. {@code com.sai.oracle.scripts.JO2P0}
     *
