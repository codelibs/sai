...
import static org.codelibs.sai.internal.ir.Node.NO_FINISH;
import static org.codelibs.sai.internal.ir.Node.NO_LINE_NUMBER;
import static org.codelibs.sai.internal.ir.Node.NO_TOKEN;
...
import org.codelibs.sai.internal.ir.SplitNode;
...
final class SplitIntoFunctions extends NodeVisitor<BlockLexicalContext> {
    ...
    public Node leaveSplitNode(final SplitNode splitNode) {
        ...
        final FunctionNode fn =
                new FunctionNode(originalFn.getSource(), body.getFirstStatementLineNumber(), newFnToken, finish, NO_TOKEN, namespace,
                        createIdent(name), originalFn.getName() + "$" + name,
                        ...
                        FunctionNode.IS_ANONYMOUS | FunctionNode.USES_ANCESTOR_SCOPE | FunctionNode.IS_SPLIT).setBody(lc, body)
        ...
        final CallNode callNode =
                new CallNode(firstLineNumber, token, finish, new AccessNode(NO_TOKEN, NO_FINISH, fn, "call"),
                        isProgram ? Arrays.<Expression> asList(thisIdent, createReturnIdent())
                                : Collections.<Expression> singletonList(thisIdent), false);
        ...
        if (splitState.hasBreak) {
            ...
            splitStateHandler =
                    makeIfStateEquals(firstLineNumber, token, finish, BREAK_STATE, enblockAndVisit(new BreakNode(NO_LINE_NUMBER, token,
                            finish, null)), splitStateHandler);
        }
    ...
    private static IfNode makeIfStateEquals(final int lineNumber, final long token, final int finish, final int value, final Block pass,
            final Statement fail) {
        return new IfNode(lineNumber, token, finish, new BinaryNode(Token.recast(token, TokenType.EQ_STRICT), GetSplitState.INSTANCE,
                intLiteral(value)), pass, fail == null ? null : new Block(NO_TOKEN, NO_FINISH, fail));
    }
    ...
    public boolean enterVarNode(final VarNode varNode) {
        ...
        if (init != null) {
            ...
            new ExpressionStatement(varNode.getLineNumber(), token, varNode.getFinish(), new BinaryNode(token, varNode.getName(),
        ...
        if (!artificialBlock) {
            if (lc.isFunctionBody()) {
                ...
                if (getCurrentFunctionState().fn.isProgram()) {
                    ...
                    lc.prependStatement(new ExpressionStatement(NO_LINE_NUMBER, NO_TOKEN, NO_FINISH, new BinaryNode(Token.toDesc(
                            TokenType.ASSIGN, 0, 0), createReturnIdent(), createReturnParamIdent())));
                }
            }
        }
        return block;
