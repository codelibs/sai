...
import static org.codelibs.sai.internal.codegen.CompilerConstants.THIS_DEBUGGER;
...
import static org.codelibs.sai.internal.codegen.CompilerConstants.staticField;
import static org.codelibs.sai.internal.codegen.CompilerConstants.virtualCallNoLookup;
import static org.codelibs.sai.internal.codegen.ObjectClassGenerator.PRIMITIVE_FIELD_TYPE;
...
import org.codelibs.sai.internal.codegen.CompilerConstants.FieldAccess;
import org.codelibs.sai.internal.codegen.types.ArrayType;
import org.codelibs.sai.internal.codegen.types.BitwiseType;
import org.codelibs.sai.internal.codegen.types.NumericType;
...
import org.codelibs.sai.internal.ir.JoinPredecessor;
...
import org.codelibs.sai.internal.runtime.ArgumentSetter;
 ...
 *
 * Running Sai with -ea will assert as soon as a type stack
 * becomes corrupt, for easier debugging
 *
 * Running Sai with -Dsai.codegen.debug=true will print
...
public class MethodEmitter {
    ...
    static {
        final String tl = Options.getStringProperty("sai.codegen.debug.trace", "-1");
        int line = -1;
        try {
     ...
     */
    private void _try(final Label entry, final Label exit, final Label recovery, final String typeDescriptor,
            final boolean isOptimismHandler) {
    ...
    MethodEmitter load(final String s) {
        ...
        //SAI-142 - split too large string
    ...
    MethodEmitter load(final Type type, final int slot) {
        ...
        assert !preventUndefinedLoad || (slot < stack.localVariableTypes.size() && stack.localVariableTypes.get(slot) != Type.UNKNOWN) : "Attempted load of uninitialized slot "
                + slot + " (as type " + type + ")";
    ...
    private MethodEmitter invoke(final int opcode, final String className, final String methodName, final String methodDescriptor,
    ...
    private void jump(final int opcode, final Label label, final int n) {
        for (int i = 0; i < n; i++) {
            assert peekType().isInteger() || peekType().isBoolean() || peekType().isObject() : "expecting integer type or object for jump, but found "
                    + peekType();
            popType();
        }
        joinTo(label);
    ...
    MethodEmitter dynamicNew(final int argCount, final int flags, final String msg) {
        ...
        method.visitInvokeDynamicInsn(
                msg != null && msg.length() < LARGE_STRING_THRESHOLD ? "dyn:new:" + NameCodec.encode(msg) : "dyn:new", signature,
                LINKERBOOTSTRAP, flags);
    ...
    MethodEmitter dynamicCall(final Type returnType, final int argCount, final int flags, final String msg) {
        ...
        method.visitInvokeDynamicInsn(msg != null && msg.length() < LARGE_STRING_THRESHOLD ? "dyn:call:" + NameCodec.encode(msg)
                : "dyn:call", signature, LINKERBOOTSTRAP, flags);
        pushType(returnType);
    ...
    */
    ...
    void dynamicSetIndex(final int flags) {
        ...
        method.visitInvokeDynamicInsn("dyn:setElem|setProp",
                methodDescriptor(void.class, receiver.getTypeClass(), index.getTypeClass(), value.getTypeClass()), LINKERBOOTSTRAP, flags);
    }
