...
final class FindScopeDepths extends SimpleNodeVisitor implements Loggable {
    ...
    public Node leaveFunctionNode(final FunctionNode functionNode) {
        ...
        final RecompilableScriptFunctionData data =
                new RecompilableScriptFunctionData(newFunctionNode, compiler.getCodeInstaller(),
                        ObjectClassGenerator.createAllocationStrategy(newFunctionNode.getThisProperties(), compiler.getContext()
                                .useDualFields()), nestedFunctions, externalSymbolDepths.get(fnId), internalSymbols.get(fnId));
    ...
    private void increaseDynamicScopeCount(final Node node) {
        ...
        ++dynamicScopeCount;
        if (log.isEnabled()) {
            log.finest(quote(lc.getCurrentFunction().getName()), " ++dynamicScopeCount = ", dynamicScopeCount, " at: ", node,
                    node.getClass());
        }
    }
