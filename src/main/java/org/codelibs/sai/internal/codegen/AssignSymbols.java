...
import static org.codelibs.sai.internal.codegen.CompilerConstants.ARGUMENTS_VAR;
import static org.codelibs.sai.internal.codegen.CompilerConstants.CALLEE;
import static org.codelibs.sai.internal.codegen.CompilerConstants.EXCEPTION_PREFIX;
import static org.codelibs.sai.internal.codegen.CompilerConstants.ITERATOR_PREFIX;
...
import static org.codelibs.sai.internal.codegen.CompilerConstants.SWITCH_TAG_PREFIX;
...
import static org.codelibs.sai.internal.ir.Symbol.HAS_OBJECT_VALUE;
import static org.codelibs.sai.internal.ir.Symbol.IS_CONST;
import static org.codelibs.sai.internal.ir.Symbol.IS_FUNCTION_SELF;
import static org.codelibs.sai.internal.ir.Symbol.IS_GLOBAL;
import static org.codelibs.sai.internal.ir.Symbol.IS_INTERNAL;
import static org.codelibs.sai.internal.ir.Symbol.IS_LET;
import static org.codelibs.sai.internal.ir.Symbol.IS_PARAM;
import static org.codelibs.sai.internal.ir.Symbol.IS_PROGRAM_LEVEL;
import static org.codelibs.sai.internal.ir.Symbol.IS_SCOPE;
import static org.codelibs.sai.internal.ir.Symbol.IS_THIS;
import static org.codelibs.sai.internal.ir.Symbol.IS_VAR;
import static org.codelibs.sai.internal.ir.Symbol.KINDMASK;
...
final class AssignSymbols extends SimpleNodeVisitor implements Loggable {
    ...
    private FunctionNode createSyntheticInitializers(final FunctionNode functionNode) {
        ...
        try {
            ...
            if (functionNode.needsArguments()) {
                ...
                syntheticInitializers.add(createSyntheticInitializer(createImplicitIdentifier(ARGUMENTS_VAR.symbolName()), ARGUMENTS,
                        functionNode));
            }
    ...
    private <T extends Node> T end(final T node, final boolean printNode) {
        if (debug) {
            ...
            sb.append("[LEAVE ").append(name(node)).append("] ").append(printNode ? node.toString() : "").append(" in '")
                    .append(lc.getCurrentFunction().getName()).append('\'');
    ...
    private Node leaveDELETE(final UnaryNode unaryNode) {
        ...
        if (rhs instanceof IdentNode) {
            ...
            final boolean failDelete =
                    strictMode || (!symbol.isScope() && (symbol.isParam() || (symbol.isVar() && !symbol.isProgramLevel())));
    ...
    public Node leaveForNode(final ForNode forNode) {
        if (forNode.isForIn()) {
            return forNode.setIterator(lc, newObjectInternal(ITERATOR_PREFIX)); //SAI-73
        }
    ...
    public Node leaveFunctionNode(final FunctionNode functionNode) {
        ...
        if (isUnparsedFunction(functionNode)) {
            ...
            finalizedFunction =
                    markProgramBlock(removeUnusedSlots(createSyntheticInitializers(finalizeParameters(lc.applyTopFlags(functionNode))))
    ...
    private Symbol newInternal(final CompilerConstants cc, final int flags) {
        return defineSymbol(lc.getCurrentBlock(), lc.getCurrentFunction().uniqueName(cc.symbolName()), null, IS_VAR | IS_INTERNAL | flags); //SAI-73
    }
    ...
    private boolean start(final Node node, final boolean printNode) {
        if (debug) {
            ...
            sb.append("[ENTER ").append(name(node)).append("] ").append(printNode ? node.toString() : "").append(" in '")
                    .append(lc.getCurrentFunction().getName()).append("'");
            log.info(sb);
            log.indent();
        }
