...
import static org.codelibs.sai.internal.ir.Expression.isAlwaysFalse;
...
final class LocalVariableTypesCalculator extends SimpleNodeVisitor {
    ...
    private enum LvarType {
        UNDEFINED(Type.UNDEFINED), BOOLEAN(Type.BOOLEAN), INT(Type.INT), DOUBLE(Type.NUMBER), OBJECT(Type.OBJECT);
    ...
    private void enterTestFirstLoop(final LoopNode loopNode, final JoinPredecessorExpression modify, final Expression iteratorValues,
            final boolean iteratorValuesAreObject) {
        final JoinPredecessorExpression test = loopNode.getTest();
        if (isAlwaysFalse(test)) {
            visitExpressionOnEmptyStack(test);
            return;
    ...
    public Node leaveFunctionNode(final FunctionNode functionNode) {
        ...
        final SimpleNodeVisitor applyChangesVisitor = new SimpleNodeVisitor() {
            ...
            public Node leaveBinaryNode(final BinaryNode binaryNode) {
                if (binaryNode.isComparison()) {
                    ...
                    case NE_STRICT:
                        ...
                        final Expression undefinedNode =
                                createIsUndefined(binaryNode, lhs, rhs, tt == TokenType.EQ_STRICT ? Request.IS_UNDEFINED
                                        : Request.IS_NOT_UNDEFINED);
            ...
            public Node leaveFunctionNode(final FunctionNode nestedFunctionNode) {
                ...
                final FunctionNode newNestedFunction = (FunctionNode) nestedFunctionNode.accept(new LocalVariableTypesCalculator(compiler));
    ...
    private void setConversion(final JoinPredecessor node, final Map<Symbol, LvarType> branchLvarTypes,
            final Map<Symbol, LvarType> joinLvarTypes) {
        if (node == null) {
            return;
        }
