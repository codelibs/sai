...
import static org.codelibs.sai.internal.parser.TokenKind.BINARY;
import static org.codelibs.sai.internal.parser.TokenKind.BRACKET;
import static org.codelibs.sai.internal.parser.TokenKind.FUTURE;
import static org.codelibs.sai.internal.parser.TokenKind.FUTURESTRICT;
import static org.codelibs.sai.internal.parser.TokenKind.IR;
import static org.codelibs.sai.internal.parser.TokenKind.KEYWORD;
import static org.codelibs.sai.internal.parser.TokenKind.LITERAL;
import static org.codelibs.sai.internal.parser.TokenKind.SPECIAL;
import static org.codelibs.sai.internal.parser.TokenKind.UNARY;
...
public enum TokenType {
    ERROR(SPECIAL, null), EOF(SPECIAL, null), EOL(SPECIAL, null), COMMENT(SPECIAL, null),
    ...
    NOT(UNARY, "!", 14, false), NE(BINARY, "!=", 9, true), NE_STRICT(BINARY, "!==", 9, true), MOD(BINARY, "%", 13, true), ASSIGN_MOD(
            ...
            BINARY, "|=", 2, false), OR(BINARY, "||", 4, true), RBRACE(BRACKET, "}"), BIT_NOT(UNARY, "~", 14, false),
    ...
    BREAK(KEYWORD, "break"),
    //  BYTE           (FUTURE,   "byte"),
    CASE(KEYWORD, "case"), CATCH(KEYWORD, "catch"),
    //  CHAR           (FUTURE,   "char"),
    CLASS(FUTURE, "class"), CONST(KEYWORD, "const"), CONTINUE(KEYWORD, "continue"), DEBUGGER(KEYWORD, "debugger"), DEFAULT(KEYWORD,
            "default"), DELETE(UNARY, "delete", 14, false), DO(KEYWORD, "do"),
    //  DOUBLE         (FUTURE,   "double"),
    //  EACH           (KEYWORD,  "each"),  // Contextual.
    ELSE(KEYWORD, "else"), ENUM(FUTURE, "enum"), EXPORT(FUTURE, "export"), EXTENDS(FUTURE, "extends"), FALSE(LITERAL, "false"),
    //  FINAL          (FUTURE,   "final"),
    ...
    //  GOTO           (FUTURE,   "goto"),
    IF(KEYWORD, "if"), IMPLEMENTS(FUTURESTRICT, "implements"), IMPORT(FUTURE, "import"), IN(BINARY, "in", 10, true), INSTANCEOF(BINARY,
            "instanceof", 10, true),
    //  INT            (FUTURE,   "int"),
    INTERFACE(FUTURESTRICT, "interface"), LET(FUTURESTRICT, "let"),
    //  LONG           (FUTURE,   "long"),
    ...
    NEW(UNARY, "new", 17, false), NULL(LITERAL, "null"), PACKAGE(FUTURESTRICT, "package"), PRIVATE(FUTURESTRICT, "private"), PROTECTED(
            FUTURESTRICT, "protected"), PUBLIC(FUTURESTRICT, "public"), RETURN(KEYWORD, "return"),
    //  SET            (KEYWORD,  "set"), // Contextual.
    //  SHORT          (FUTURE,   "short"),
    STATIC(FUTURESTRICT, "static"), SUPER(FUTURE, "super"), SWITCH(KEYWORD, "switch"),
    //  SYNCHRONIZED   (FUTURE,   "synchronized"),
    THIS(KEYWORD, "this"), THROW(KEYWORD, "throw"),
    //  THROWS         (FUTURE,   "throws"),
    //  TRANSIENT      (FUTURE,   "transient"),
    TRUE(LITERAL, "true"), TRY(KEYWORD, "try"), TYPEOF(UNARY, "typeof", 14, false), VAR(KEYWORD, "var"), VOID(UNARY, "void", 14, false),
    ...
    DECIMAL(LITERAL, null), HEXADECIMAL(LITERAL, null), OCTAL_LEGACY(LITERAL, null), OCTAL(LITERAL, null), BINARY_NUMBER(
            LITERAL, null), FLOATING(LITERAL, null), STRING(LITERAL, null), TEMPLATE(LITERAL, null), TEMPLATE_HEAD(
            LITERAL, null), TEMPLATE_MIDDLE(LITERAL, null), TEMPLATE_TAIL(LITERAL, null), ESCSTRING(
            LITERAL, null), EXECSTRING(LITERAL, null), IDENT(LITERAL, null), REGEX(LITERAL, null), XML(LITERAL, null), OBJECT(LITERAL, null), ARRAY(
            LITERAL, null),

    COMMALEFT(IR, null), DECPOSTFIX(IR, null), INCPOSTFIX(IR, null);
